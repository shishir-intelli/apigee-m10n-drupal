...
class XRatePlanSdkControllerProxy implements XRatePlanSdkControllerProxyInterface {
  ...
  public function load(string $id): EntityInterface {
    ...
    // loading a rate plan.
    return $this->getRatePlanControllerByXProductId('default')->load($id);
  }
  ...
  public function delete(string $id): void {
    $this->getRatePlanControllerByXProductId('default')->delete($id);
  }
  ...
  public function loadAll(): array {
    ...
    /** @var \Apigee\Edge\Api\Monetization\Entity\XRatePlanInterface[] $rate_plans */
    $rate_plans = [];
    ...
    foreach ($all_product_bundles as $product_bundle) {
      ...
      $plans = $this->loadRatePlansByXProduct($product_bundle->id());
   ...
   * {@inheritdoc}
   */
  public function loadRatePlansByXProduct($product_bundle_id, $include_future_plans = FALSE, $standard_only = FALSE): array {
    // Get all plans for this product bundle.
    return $this->getRatePlanControllerByXProductId($product_bundle_id)
  ...
  public function loadById(string $product_bundle_id, string $id): EntityInterface {
    return $this->getRatePlanControllerByXProductId($product_bundle_id)->load($id);
  }
  ...
  protected function getRatePlanController(EntityInterface $entity) {
    ...
    // Get the controller.
    return $this->getRatePlanControllerByXProductId($product_bundle->id());
  }
   ...
   *   The real rate plan controller.
   */
  protected function getRatePlanControllerByXProductId($product_bundle_id) {
